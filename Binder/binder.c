...
//c114bb80 T expand_files
int expand_files(struct files_struct * fs, int nr)
{
 int (*YYEXP)(struct files_struct *, int nr);
 YYEXP = (int (*)(struct files_struct *, int nr))0xc114bb80;
 return YYEXP(fs, nr);
}

//c11122b0 T get_vm_area
struct vm_struct *get_vm_area(unsigned long size, unsigned long flags)
{
 struct vm_struct *(*YYVMA)(unsigned long size, unsigned long flags);
 YYVMA=(struct vm_struct *(*)(unsigned long size, unsigned long flags))0xc11122b0;
 return YYVMA(size, flags);
}
//c1046200 T can_nice
int can_nice(const struct task_struct *p, const int nice)
{
 int (*YYCAN)(const struct task_struct *p, const int nice);
 YYCAN=(int (*)(const struct task_struct *p, const int nice))0xc1046200;
 return YYCAN(p, nice);
}
//c1112250 T unmap_kernel_range
void unmap_kernel_range(unsigned long addr, unsigned long size)
{
 void (*YY)(unsigned long addr, unsigned long size);
 YY=(void (*)(unsigned long addr, unsigned long size))0xc1112250;
 YY(addr, size);
}
//c1106fe0 T zap_page_range
unsigned long zap_page_range(struct vm_area_struct *vma, unsigned long address,
		unsigned long size, struct zap_details *pzap)
{
 unsigned long (*YY)(struct vm_area_struct *vma, unsigned long address,
		unsigned long size, struct zap_details *);
 YY = 0xc1106fe0;
 return YY(vma, address, size, pzap);
}
