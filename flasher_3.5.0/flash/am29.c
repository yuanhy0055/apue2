/*
 * am29.c: algorithm for AMD AM29Fxx
 *
...
int data_polling_algorithm_am29(struct flash_hw * flash_hw,__u32 addr,__u8 byte, __u32 wait)
{
__u8 data;
...
int toggle_bit_algorithm_am29(struct flash_hw * flash_hw,__u32 wait)
{
...
// look for finish of erasing
	...
	if (toggle_bit_algorithm_am29(flash_hw, flash_hw->flash->c_erase_timeout))
	{
...
int program_byte_am29(struct flash_hw *flash_hw,__u32 addr,__u8 byte)
{
...
return data_polling_algorithm_am29(flash_hw,addr,byte,flash_hw->flash->prg_timeout);
}

/* ------------------------------------------------------------------------ */

ssize_t flash_write_data_am29(struct flash_hw *flash_hw,struct file * file,
...
// erase flash
	...
	for ( addr=0; addr < count; addr++)
	{
		...
		if ( program_byte_am29(flash_hw,addr+flash_hw->curr_pos, byte) != byte )
			return -ENXIO;
	}
