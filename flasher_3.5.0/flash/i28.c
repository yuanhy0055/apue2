...
__u16 id;
	...
	chip_reset_i28(flash_hw);

	chip_put_byte( flash_hw, 0x90, offset );
...
__u16 autoselect_i28(struct flash_hw *flash_hw) {return _autoselect(flash_hw,0);};
...
int program_byte_i28(struct flash_hw *flash_hw,__u32 addr,__u8 byte)
{
int i;
...
ssize_t flash_write_data_i28(struct flash_hw *flash_hw, struct file * file,
...
// erase flash
	...
	for ( addr=0; addr < count; addr++)
	{
		...
		if ( program_byte_i28(flash_hw, addr+flash_hw->curr_pos, byte) != byte )
			return -ENXIO;
	}
