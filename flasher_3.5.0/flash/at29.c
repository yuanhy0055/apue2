/*
 * at29.c: algorithm for ATMEL AT29Cxxx
 *
...
__u16 autoselect_at29(struct flash_hw *flash_hw) {return _autoselect(flash_hw,0);};
...
// check not supported
        ...
        if (flash_hw->flash->block_address[0] < 0)
	    return 0;
...
int program_byte_at29(struct flash_hw *flash_hw,__u32 addr,__u8 * byte)
{
__u32 flags;
__u32 i;
...
ssize_t flash_write_data_at29(struct flash_hw *flash_hw,struct file * file,
...
// erase flash
	...
	if ( program_byte_at29(flash_hw,flash_hw->curr_pos, byte) )
		return -ENXIO;
...
int sdp_enable_at29(struct flash_hw *flash_hw, char * secret)
{
__u32 flags;
int i=0;
...
int sdp_disable_at29(struct flash_hw *flash_hw, char * secret)
{
__u32 flags;
int i=0;
...
int bbl_low_at29(struct flash_hw *flash_hw)
{
	...
	chip_put_byte( flash_hw, 0xAA, 0x5555 );
	chip_put_byte( flash_hw, 0x55, 0x2AAA );
	chip_put_byte( flash_hw, 0x40, 0x5555 );
	chip_put_byte( flash_hw, 0x00, 0x0000 );
...
int bbl_high_at29(struct flash_hw *flash_hw)
{
