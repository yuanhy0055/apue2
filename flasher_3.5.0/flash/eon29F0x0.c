/*
 * eon29F0x0.c: algorithm for EON EN29F0x0
 *
...
int data_polling_algorithm_eon29F0x0(struct flash_hw * flash_hw,__u32 addr,__u8 byte, __u32 wait)
{
__u8 data;
...
int toggle_bit_algorithm_eon29F0x0(struct flash_hw * flash_hw,__u32 wait)
{
...
__u32 id;
	...
	chip_reset_eon29F0x0(flash_hw);

	chip_put_byte( flash_hw, 0xAA, 0x555 );
	chip_put_byte( flash_hw, 0x55, 0x2AA );
...
// look for finish of erasing
	...
	if (toggle_bit_algorithm_eon29F0x0(flash_hw, flash_hw->flash->c_erase_timeout))
	{
...
int program_byte_eon29F0x0(struct flash_hw *flash_hw,__u32 addr,__u8 byte)
{
...
return data_polling_algorithm_eon29F0x0(flash_hw,addr,byte,flash_hw->flash->prg_timeout);
}

/* ------------------------------------------------------------------------ */

ssize_t flash_write_data_eon29F0x0(struct flash_hw *flash_hw,struct file * file,
...
// erase flash
	...
	for ( addr=0; addr < count; addr++)
	{
		...
		if ( program_byte_eon29F0x0(flash_hw,addr+flash_hw->curr_pos, byte) != byte )
			return -ENXIO;
	}
